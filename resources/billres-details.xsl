...
<!--  VERSION:   4.2                                               -->
<!--  DATE:      Septmber 11, 2020                                     -->
<!--  Previous version and date: 4.1 July 23, 2019                  -->
<!-- ============================================================= -->
...
P.lbexBreakHere {page-break-before: always}
	...
	<xsl:template name="displayEnumTitle">
		...
		<xsl:choose>
			...
			<xsl:when
				...
				<xsl:choose>
					<xsl:when
                        test="contains(ancestor::division/@reported-display-style, 'brackets')">
						<span class="lbexBrakets">
							<xsl:text>[</xsl:text>
						</span>
						<span class="lbexTitleLevelTrad">
	...
	<xsl:template name="displayEnumDivision">
		...
		<xsl:choose>
			...
			<xsl:when test="contains($aCurrElementStyle ,'codification')">
				<span class="lbexTitleLevelOLC">
					<xsl:call-template name="actualDisplayDivisionCodification"/>
				</span>
			</xsl:when>
			<xsl:otherwise>
				<span class="lbexTitleLevelOLC">
	...
	<xsl:template name="actualDisplayDivisionCodification">
		<xsl:call-template name="ActualPrintHeadingOf">
			<xsl:with-param name="aTextToAdd">
				<xsl:text>Division </xsl:text>
			</xsl:with-param>
	...
	<xsl:template name="displaySubdivisionBody">
		...
		<xsl:choose>
			<xsl:when
				test="$aCurrElementStyle = 'OLC' or $aCurrElementStyle = 'USC' or $aCurrElementStyle = 'tax' or contains($aCurrElementStyle, 'codification')">
				<span class="lbexTitleLevelOLC">
			...
			<xsl:when
				test="$aCurrElementStyle = 'traditional-inline' or $aCurrElementStyle = 'appropriations'or contains($aCurrElementStyle, 'codification')">
				<span class="lbexSubChapterLevelOLC">
					<xsl:call-template name="actualDisplaySubdivisionBody">
						<xsl:with-param name="aCurrElementStyle" select="$aCurrElementStyle"/>
					</xsl:call-template>
			...
			<xsl:when
				...
				<p class="lbexIndent">
					<xsl:if test="@section-type = 'subsequent-section'">
                       ...
                       <!--TASK #11801 FY2020-->
					<xsl:if test="@section-type = 'section-one'">
						<xsl:apply-templates select="enum"/>
					</xsl:if>
					<xsl:text> </xsl:text>
					<xsl:apply-templates select="text"/>
			...
			<xsl:when test="$currentElementStyle = 'OLC' and $isIdReferenceExists = 'no'">
				...
				<xsl:choose>
					<xsl:when test="child::quoted-block">
                           <!--TASK#11392 FY2020-->
							<xsl:choose>
								<xsl:when test="local-name(child::quoted-block/following-sibling::quoted-block/child::section/child::quoted-block)='quoted-block'"></xsl:when>
								<xsl:otherwise>
									<xsl:apply-templates select="child::quoted-block/following-sibling::*"/>
								</xsl:otherwise>
							</xsl:choose>
	...
	<xsl:template name="NestedOpenQB">
		<xsl:choose>
			<xsl:when test="(ancestor::quoted-block[1]/@other-style='codification-quoted' or not(ancestor::quoted-block[1]/@other-style)) and (ancestor::quoted-block[2]/@other-style = 'codification-quoted' or not(ancestor::quoted-block[2]/@other-style))">
				<xsl:text>“ &#8216;</xsl:text>
			</xsl:when>
			<xsl:when test="(ancestor::quoted-block[1]/@other-style='codification-not-quoted' or not(ancestor::quoted-block[1]/@other-style)) and (ancestor::quoted-block[2]/@other-style = 'codification-quoted' or not(ancestor::quoted-block[2]/@other-style))">
				<xsl:text>“ </xsl:text>
			</xsl:when>
			<xsl:when test="(ancestor::quoted-block[1]/@other-style='codification-quoted' or not(ancestor::quoted-block[1]/@other-style)) and (ancestor::quoted-block[2]/@other-style = 'codification-not-quoted' or not(ancestor::quoted-block[2]/@other-style))">
				<xsl:text>“ </xsl:text>
			</xsl:when>
			<xsl:when test="ancestor::quoted-block[1]/@other-style='codification-not-quoted' and ancestor::quoted-block[2]/@other-style = 'codification-not-quoted'">
				<xsl:text> </xsl:text>
			</xsl:when>
			<xsl:otherwise></xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<xsl:template name="NestedQBNoCodif">
		<xsl:choose>
			<xsl:when test="(ancestor::quoted-block[1]) and (ancestor::quoted-block[2])">
				<xsl:text>“ &#8216;</xsl:text>
			</xsl:when>
	...
	<xsl:template name="NestedCloseQB">
		<xsl:choose>
			<xsl:when test="(ancestor::quoted-block[1]/@other-style='codification-quoted' or not(ancestor::quoted-block[1]/@other-style))  and (ancestor::quoted-block[2]/@other-style = 'codification-quoted' or not(ancestor::quoted-block[2]/@other-style))">
				<xsl:if test="following::after-quoted-block[1]!=''">
					<xsl:value-of select="following::after-quoted-block[1]"/>
				</xsl:if>
				<xsl:text>&#8221;</xsl:text>
				<xsl:if test="following::after-quoted-block[2]!=''">
	...
	<xsl:template name="codification">
		<xsl:choose>
			<xsl:when
				test="contains(ancestor::quoted-block/@other-style,'codification')">
				<xsl:text>yes</xsl:text>
			</xsl:when>
			<xsl:otherwise>
				<xsl:text>no</xsl:text>
	...
	<xsl:template name="openQuotes">
		...
		<xsl:choose>
			<xsl:when
				test="ancestor::quoted-block and contains(@display-inline, 'yes-display-inline')">
				<xsl:if test="not(preceding-sibling::enum) and $isNestedQB='yes'">
					<xsl:choose>
						<xsl:when test="parent::quoted-block/@display-inline='no-display-inline'">
							<xsl:choose>
								<xsl:when test= "$isCodificationStyle='yes' and parent::quoted-block/@other-style='codification-not-quoted'">
									<xsl:text>“</xsl:text>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>“ ‘</xsl:text>
						...
						<xsl:when test ="local-name(ancestor::*[3])='quoted-block' and local-name(ancestor::*[2])='constitution-article' and local-name(ancestor::*[1])='section'
							and ancestor::quoted-block/@display-inline='no-display-inline'">
							<xsl:text>“ ‘</xsl:text>
						</xsl:when>
						<xsl:otherwise>
							<xsl:text> ‘</xsl:text>	
				...
				<xsl:choose>
					<xsl:when test="ancestor::amendment-block and not(ancestor::amendment-block/text) and $isNestedQB='no' ">
						<xsl:choose>
							<xsl:when test="local-name(parent::*[1])!='quoted-block' and @display-inline='yes-display-inline' and @commented='no'">	
							</xsl:when>
							<xsl:otherwise>
								<xsl:text> “</xsl:text>
							</xsl:otherwise>
					...
					<xsl:when test="parent::quoted-block/@display-inline='no-display-inline' and $isNestedQB='no' and not(preceding-sibling::enum) and local-name()='text' and @display-inline='yes-display-inline'">	
						<xsl:choose>
							<xsl:when test= "$isCodificationStyle='no'">
								<xsl:text>“</xsl:text>
							</xsl:when>
						</xsl:choose>
					</xsl:when>
					<xsl:when test="parent::quoted-block/@display-inline='yes-display-inline' and $isNestedQB='no' and not(preceding-sibling::enum) and local-name()='text' and @display-inline='yes-display-inline'">	
						<xsl:choose>
							<xsl:when test= "$isCodificationStyle='no'">
								<xsl:text>“</xsl:text>
							</xsl:when>
			...
			<xsl:when test="parent::quoted-block/@display-inline='yes-display-inline' and contains(@display-inline, 'no-display-inline') and not(preceding-sibling::enum) and $isNestedQB='no' and local-name()='text'">
				<xsl:if test="$isCodificationStyle='no'">
					<xsl:text>“</xsl:text>
				</xsl:if>
			</xsl:when>
			<xsl:when test="parent::quoted-block/@display-inline='no-display-inline' and contains(@display-inline, 'no-display-inline') and not(preceding-sibling::enum) and $isNestedQB='no' and local-name()='text'">
				<xsl:if test="$isCodificationStyle='no'">
					<xsl:text>“</xsl:text>
				</xsl:if>
			</xsl:when>
			...
			test="ancestor::quoted-block and ancestor::quoted-block/descendant::quoted-block = ancestor::quoted-block">
				...
				<xsl:choose>
					<xsl:when test="$isCodificationStyle='no'">
						<xsl:call-template name="NestedQBNoCodif"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:call-template name="NestedOpenQB"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:otherwise>
                <!--Implementation of codification style quotes and non quoted FY2020 Parent Task 10500-->
				<xsl:choose>
					<xsl:when test="ancestor::quoted-block/@other-style ='codification-not-quoted' and local-name()!='quote'"></xsl:when>
					<xsl:otherwise>
						<xsl:choose>
							<!--TASK #11357 FY2020-->
							<xsl:when test="$isNestedQB='no' and $isCodificationStyle ='yes' and ancestor::quoted-block/@other-style='codification-quoted'
								 and local-name()='quote' and local-name(child::short-title)='short-title'">
									<xsl:text>‘</xsl:text>
							</xsl:when>
							<xsl:when test="$isNestedQB='no' and $isCodificationStyle ='no' and not(ancestor::quoted-block/@other-style)
								and local-name()='quote' and local-name(child::short-title)='short-title'">
								<xsl:if test="ancestor::quoted-block">
									<xsl:text>‘</xsl:text>
								</xsl:if>
	...
	<xsl:template name="closeQuotes">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:if test="ancestor::quoted-block[1] and not(ancestor::quoted-block[2])">
					<xsl:choose>
						<xsl:when test="ancestor::quoted-block[1]/@other-style!='codification-not-quoted' and not(following::quoted-block)">
							<xsl:text>&#8221;</xsl:text>
						</xsl:when>
						<xsl:when test="ancestor::quoted-block[1]/@other-style!='codification-not-quoted' and following::node()[3]!='quoted-block'">
							<xsl:text>&#8221;</xsl:text>
						</xsl:when>
						<xsl:when test="$isCodificationStyle ='no' and local-name(following-sibling::*[1])='' and local-name(following::node()[1])='after-quoted-block'">
							<xsl:text>&#8221;</xsl:text>
						</xsl:when>
					</xsl:choose>
				</xsl:if>
	...
	<xsl:template name="getCurrentElementStyle">
		<xsl:choose>
			...
			<xsl:when
                test="not(ancestor-or-self::title[1]/@style) and ancestor-or-self::division[1]/@style">
				<xsl:choose>
					<xsl:when test="ancestor-or-self::division[1]/@style = 'other'">
						<xsl:value-of select="ancestor-or-self::division[1]/@other-style"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="ancestor-or-self::division[1]/@style"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
	...
	<xsl:template name="ActualPrintText">
		...
		<xsl:choose>
			<xsl:when
				...
				<xsl:choose>
					<xsl:when test="ancestor::appropriations-major or ancestor::appropriations-intermediate or ancestor::appropriations-small"> </xsl:when>
					<xsl:otherwise>
						<xsl:call-template name="openBracketStyles"/>
					</xsl:otherwise>
				</xsl:choose>
		...
		<xsl:if
			test="parent::*/@commented = 'yes' and (parent::*[1]/@indent != 'up1' or not(parent::*[1]/@indent))">
			<xsl:choose>
				<xsl:when test="@commented = 'no'"> </xsl:when>
				<xsl:otherwise>
					<xsl:call-template name="closedBracketStyles"/>
				</xsl:otherwise>
			</xsl:choose>
	...
	<xsl:template name="ApplyTemplWithSimpleIndentP">
		...
		<p style="text-align:left" class="lbexIndent">
			...
			<xsl:if
				...
				<xsl:choose>
					<xsl:when
						test="(not(@reported-display-style) and (not(@changed) or @changed != 'not-changed') and contains(ancestor::*/@reported-display-style, 'brackets')) and local-name() != 'list' and local-name() = 'quoted-block'">
						<xsl:text></xsl:text>
					</xsl:when>
					<xsl:otherwise>
						<xsl:call-template name="openBracketStyles"/>
